...
static char keymap[][4] = {
        ...
        /* 0x52 */{'0', '+',false, false}, // pad 0 - Insert
        /* 0x53 */{INV, 0x7F,false, false},// pad . - Delete
...
void keymap_handler(int idt_index) {
    uchar ext = 2; // keymap 状态索引，默认没有shift键

    // 告诉中断控制芯片中断已处理(好像不做这个事情也没啥问题)
    send_eoi(idt_index);
